...
class UpdateCrossBuildDependencySatisfactionTask(BaseTask):
    """
    Fetches source package cross installability results from qa.debian.org/dose
    """

    class Scheduler(IntervalScheduler):
        interval = 3600 * 3

    BASE_URL = 'https://qa.debian.org/dose/debcheck/' \
               'cross_unstable_main_amd64/latest'
    ACTION_ITEM_TYPE_NAME = 'debian-crossbuilddependency-satisfaction'
    ACTION_ITEM_TEMPLATE = \
        'debian/crossbuilddependency-satisfaction-action-item.html'

    def __init__(self, force_update=False, *args, **kwargs):
        super(UpdateCrossBuildDependencySatisfactionTask,
    ...
    def get_dependency_satisfaction(self):
        ...
        for i, line in enumerate(response.iter_lines(decode_unicode=True)):
            ...
            if not arches:
                continue
            # if the source package only builds arch:all binary packages, don't
            # report the problem because arch:all binary packages do not need
    ...
    def update_action_item(self, package, unsats):
        ...
        action_item.short_description = \
            "source package has {count} unsatisfiable " \
            "cross build dependenc{plural}".format(
                count=len(unsats),
                plural='y' if len(unsats) == 1 else 'ies',
            )
        action_item.extra_data = {
    ...
    def execute(self):
        ...
        with transaction.atomic():
            PackageData.objects.filter(
                key='crossbuilddependency_satisfaction').delete()
            ...
            for name, unsats in dep_sats.items():
                ...
                dep_sat_info = PackageData(
                    ...
                    package=package,
                    value={'crossbuilddependency_satisfaction': unsats})
                pkgdata_list.append(dep_sat_info)
